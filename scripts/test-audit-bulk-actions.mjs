...
const ptBRSource = await fs.readFile(path.resolve('src/i18n/pt-BR.json'), 'utf8')
...
const jsonActionEnd = popupSource.indexOf(']', jsonActionStart)
...
assert.match(popupSource.slice(jsonActionStart, jsonActionEnd), /type: 'primary'/)
assert.doesNotMatch(popupSource.slice(highlightActionStart, priorityActionStart), /type: 'primary'/)

assert.doesNotMatch(violationsListSource, /Triagem do achado|Confirmar problema/)
...
assert.match(ptBRSource, /Achados acionáveis/)
assert.match(ptBRSource, /Variação visual simulada/)
assert.match(ptBRSource, /Simulador de percepção visual/)
assert.doesNotMatch(ptBRSource, /Tipo de deficiência visual|Ação necessária/)
