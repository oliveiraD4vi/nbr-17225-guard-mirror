...
O escopo aqui documentado contém 146 itens revisados contra a referência pública da ABNT NBR 17225.
...
| Recomendações normativas  |                          50 |
| Incorporadas no lote V2.x | 34 recomendações adicionais |
...
## Recomendações incorporadas no lote V2.x

As 34 recomendações antes mantidas como backlog estão implementadas em `src/rules/remaining-recommendations.ts`. Elas usam verificações assistidas ou roteiros manuais e estão registradas em `FUTURE_RULES_PLAN.md`.
...
```text
Verificação de regras concluída: 146 itens documentados mapeados para 146 implementações: 96 requisitos e 50 recomendações.
```
